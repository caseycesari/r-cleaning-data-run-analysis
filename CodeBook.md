...
The analysis script, `run_analysis.R` reads in the processed experiment data and performs a number of steps to get into summary form.

 - Both the process test and training datasets are read in an merged into one data frame.
 ...
 - Invalid characters ("()" and "-" in this case) are removed from the column names. Also, columns that contain the phrase "BodyBody" are replaced with "Body".
...
Each line in `run_analysis.R` is commented. Reference the file for more information.
...
All of the following fields represent the mean of recorded data point for the subject and activity. The detailed description of different measurement types can be found in the `features_info.txt` file included in the data [zip file](https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip).
